...
<body>
  <div class="container">
    ...
    <input type="number" id="amount" placeholder="Old token amount" />
    <br>
  ...
  <script>
    ...
    async function approve() {
      ...
      try {
        await token.methods.approve(migratorAddress, web3.utils.toWei(amount, "ether")).send({ from: account });
    ...
    async function migrate() {
      ...
      try {
        await migrator.methods.migrate(web3.utils.toWei(amount, "ether")).send({ from: account });
